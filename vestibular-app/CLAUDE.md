...
## Módulo de importação de provas da UERJ (jul/2026)
- **Pipeline offline** em `scripts/importador_uerj/` (Python): crawler BFS do vestibular.uerj.br (descobre anos sozinho; o padrão `/anexos/AAE/` dá ano+fase: E=1→1º EQ, 2→2º EQ, 3→Discursivo), download paralelo com SHA-256/retry/validação, extração de questões (PyMuPDF→pdfplumber→OCR opcional; regex `QUESTÃO NN` + alternativas `(A)…(D)`), classificação heurística (sem match ⇒ "Não Classificada") e publicação via PostgREST/Storage. Ver `scripts/importador_uerj/README.md`. Requer `truststore` (o certificado do site não valida com certifi).
- **Tabelas** (parte 7 do `supabase_migration.sql`): `provas_uerj`, `questoes_uerj`, `uerj_import_logs` — leitura pública, escrita admin/service-role. ⚠️ O frontend degrada com elegância se a migration não tiver rodado (404 silencioso → listas vazias).
- **Onde aparece no app**: pastas do hub de Questões (resolver com correção quando há gabarito; sem gabarito registra a resposta e avisa), Biblioteca UERJ (provas importadas viram cards junto às dos professores) e aba **Provas UERJ** do Painel do Admin (`GerenciarProvasUerj.jsx`: métricas, logs, correção de classificação, exclusão, guia de sincronização).
- Testado contra o site real (jul/2026): crawl de 120 páginas → 1.208 PDFs catalogados, 100 provas identificadas; extração validada no 1º/2º EQ 2026 (58 questões, 4 alternativas) e discursiva de Física (10 questões).
