...
- Validação (jul/2026, dataset local completo): 58 provas de vestibular (2012–2027), 1.947 questões (100% com 0 ou 4 alternativas), 1.214 com gabarito oficial, zero PDFs de outros certames.
...
## Segurança (jul/2026)
- **RLS revisada (parte 9 do `supabase_migration.sql`)**: `materiais_estudo` e Storage (`materiais`) só aceitam escrita/exclusão de **admin** (antes qualquer aluno logado podia publicar material e apagar PDFs do bucket!); `profiles` só leitura do próprio cargo (escrita apenas via RPCs SECURITY DEFINER); `cronogramas` e `questoes_respondidas` por dono. As políticas antigas dessas tabelas são dropadas dinamicamente (via `pg_policies`) antes de recriar.
