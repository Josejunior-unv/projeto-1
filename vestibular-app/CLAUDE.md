...
- `QuestoesHub.jsx` — aba "Questões": hub com o Banco ENEM em destaque (abre `QuestoesEnem`) + pastas por matéria com as **questões da UERJ importadas** (contagens reais via `contarQuestoesPorDisciplina`; pasta com conteúdo abre `QuestoesUerj.jsx`, que reusa o `QuestaoCard` e grava acertos em `questoes_respondidas`). Estado da pasta persistido em `questoes_pasta`.

## Módulo de importação de provas da UERJ (reescrito em jul/2026)
- **Pipeline offline** em `scripts/importador_uerj/` (Python): crawler BFS do vestibular.uerj.br, download paralelo com SHA-256/retry/validação, extração de questões, classificação e publicação via PostgREST/Storage. Ver `scripts/importador_uerj/README.md`. Requer `truststore` (o certificado do site não valida com certifi).
- **Sinais de ano/fase, em ordem**: padrão `/anexos/AAE/` (E=1→1º EQ, 2→2º EQ, 3→Discursivo — mas há anexos com id de 3 dígitos que NÃO seguem o padrão, ex. 223=Vestibular 2022) → **nome do arquivo** (`2015_1eq_prova.pdf`; normalizar `_`/`-` para espaço antes do regex, senão o `\b` não casa) → texto do link → conteúdo do PDF ("Vestibular Estadual AAAA"). ⚠️ NUNCA usar a pasta `wp-content/uploads/AAAA/MM/` como ano (é a data de upload: provas de 2012–2020 moram em uploads/2019/).
- **Extração** (`extrair.py`): blocos reordenados por coluna (PDFs de 2 colunas embaralhavam a ordem e fundiam questões), rótulos de margem "QUESTÃO"+"NN" fundidos (layout ≤2020), números de linha da margem descartados, **área/idioma lidos do rodapé de cada página** ("Linguagens Espanhol", "Ciências Humanas"...), banners "AS QUESTÕES X A Y REFEREM-SE..." movidos para a questão seguinte, alternativas aceitas só em sequência estrita A→E. PDFs com texto que não imprimem "Vestibular Estadual" são de OUTROS certames (CBMERJ/proficiência/mestrado) e ficam fora. Provas 2012 têm numeração vetorial (sem texto) → entram só como PDF.
- **Questões de língua estrangeira**: 23–27 saem em inglês/espanhol/francês (3 versões, mesma numeração). Todas são importadas (unicidade = prova+numero+disciplina, parte 8 da migration) e **nunca recebem gabarito** (cada idioma tem coluna própria no gabarito; coluna errada = corrigir o aluno com a resposta de outra língua).
- **Gabaritos**: casados por (ano, fase) do NOME do arquivo + fallback pela mesma pasta de anexo; retificados aplicados por último; extrator espacial + fallback por sequência (layout 2024+: coluna de números seguida de coluna de letras).
- **Publicação**: upsert de provas por hash + substituição total das questões da prova + remoção de provas obsoletas do banco (execuções completas). Reimportar troca IDs de questões (progresso local do banco UERJ zera; `questoes_respondidas` permanece).
- **Tabelas** (partes 7 e 8 do `supabase_migration.sql`): `provas_uerj`, `questoes_uerj` (+coluna `area`, unique por prova+numero+disciplina), `uerj_import_logs` (política de denúncia de questão por alunos) — leitura pública, escrita admin/service-role. ⚠️ O frontend degrada com elegância se a migration não tiver rodado.
- **Onde aparece no app**: Central UERJ no hub de Questões (banco com filtros combináveis — área/disciplina/assunto/ano/tipo/dificuldade/status —, provas completas online, simulados, estatísticas), Biblioteca UERJ e aba **Provas UERJ** do Painel do Admin.
- **EM PRODUÇÃO desde 08/07/2026** (migration completa rodada pelo dono + republicação com service key): 108 provas (2012–2027), 1.947 questões (100% com 0 ou 4 alternativas), 1.214 com gabarito oficial, ~87% com área preenchida, 297 "Não Classificada" (corrigíveis na aba admin), zero duplicatas/órfãs/lixo. As 38 provas de concursos (CBMERJ etc.) importadas por engano na 1ª versão foram REMOVIDAS do banco. 2012 ficou só como PDF (numeração vetorial).
- **Modo degradado**: se a parte 8 da migration não existir no banco, o publicador detecta (sonda a coluna `area` via PostgREST) e publica sem `area` + 1 idioma por número, com aviso no log — nunca falha por schema desatualizado.
- ⚠️ A service role key foi colada no chat de 08/07/2026 — o dono foi orientado a rotacioná-la (Settings → API → Reset service_role key). Confirmar na próxima sessão se rotacionou.
- ⚠️ **Supabase limita cada resposta a 1000 linhas**: toda consulta que pode passar disso usa `coletarTudo()` (paginação em `questoesUerjService.js`/`uerjEstudoService.js`/`estatisticas.js`). Nunca confiar em `.range(0, 4999)`.
...
## Segurança (jul/2026)
- **RLS revisada (parte 9 do `supabase_migration.sql`, JÁ RODADA em produção em 08/07/2026)**: `materiais_estudo` e Storage (`materiais`) só aceitam escrita/exclusão de **admin** (antes qualquer aluno logado podia publicar material e apagar PDFs do bucket!); `profiles` só leitura do próprio cargo (escrita apenas via RPCs SECURITY DEFINER); `cronogramas` e `questoes_respondidas` por dono. As políticas antigas dessas tabelas são dropadas dinamicamente (via `pg_policies`) antes de recriar. Nota: a parte 1b ainda cria a política permissiva de materiais e a 9a substitui logo depois — o script deve SEMPRE rodar inteiro (o dono já colou só metade uma vez e a migration ficou incompleta).
- **Headers HTTP** no `vercel.json`: CSP (`script-src 'self'` — por isso o script anti-flash do tema mora em `public/tema.js`, NÃO inline no index.html), HSTS, nosniff, X-Frame-Options DENY, Referrer-Policy, Permissions-Policy. `connect-src` libera apenas o Supabase (https+wss) e api.enem.dev; ao adicionar API nova, atualizar a CSP.
- Busca do banco UERJ sanitiza curingas do ILIKE (`%`, `_`) e limita a 120 chars; denúncia de questão limitada a 300 chars no app e 2000 bytes na policy.
