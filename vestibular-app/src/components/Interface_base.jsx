...
function InterfaceBase({ cronograma, userId }) {
  ...
  useEffect(() => {
    ...
    const canal = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'materiais_estudo' },
      ...
      <aside className="w-64 bg-gray-900 border-r border-gray-800 p-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-2 mb-8 px-2 cursor-default group">
            ...
            <span className="font-bold text-xl tracking-wide bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">Portal do Estudante</span>
          </div>
      ...
      <main className="flex-1 p-10 overflow-y-auto bg-gradient-to-br from-gray-950 via-gray-950 to-gray-900">
        ...
        {abaAtiva === 'tarefas' && (
          <div className="max-w-3xl">
            <h1 className="text-3xl font-bold text-gray-100">Minhas Tarefas</h1>
        ...
        {abaAtiva === 'simulados' && (
          <div className="max-w-3xl">
            <h1 className="text-3xl font-bold text-gray-100">Simulados & Notas</h1>
            <p className="text-sm text-gray-400 mt-1">Acompanhe seu desempenho nos simulados.</p>
            <div className="mt-8 p-6 bg-gray-900/50 border border-gray-800 rounded-xl text-center text-gray-500 shadow-inner">
        ...
        {abaAtiva === 'enem' && (
          <div className="max-w-3xl transition-all duration-500">
            <div className="mb-6 cursor-default">
              <h1 className="text-3xl font-bold text-gray-100">Banco de Questões ENEM</h1>
              <p className="text-sm text-gray-400 mt-1">Treine seus conhecimentos com o banco de dados oficial da comunidade.</p>
            </div>
            <QuestoesEnem />
          </div>
        )}
        ...
        {abaAtiva === 'materiais' && (
          <div className="max-w-3xl animate-in fade-in duration-500">
            <h1 className="text-3xl font-bold text-gray-100 mb-2">Materiais de Apoio</h1>
            <p className="text-sm text-gray-400 mb-8">Arquivos e links disponibilizados para seus estudos.</p>
            <div className="grid gap-4">
              {materiais.map((m) => (
                <a key={m.id} href={m.url_arquivo} target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="font-semibold text-gray-200 group-hover:text-white">{m.titulo}</span>
                  <span className="text-xs font-bold bg-blue-500/10 text-blue-400 px-3 py-1.5 rounded-md border border-blue-500/20 group-hover:bg-blue-600 group-hover:text-white">Baixar</span>
                </a>
              ))}
            </div>
          </div>
